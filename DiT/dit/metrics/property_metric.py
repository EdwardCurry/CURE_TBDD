...
tasktype_name = {
    ...
    'CO2': 'regression',
    'smiles': "others",
    'smiles-l1000-mcf7-wopoint-600': 'others',
    'smiles-l1000-mcf7-wopoint': 'others',
}
...
def calculateSA(smiles_list):
    scores = []
    for i, smiles in enumerate(smiles_list):
        mol = Chem.MolFromSmiles(smiles)
        score = rdMolDescriptors.CalcSAScore(mol)
        scores.append(score)
    return np.float32(scores)

def calculateQED(smiles_list):
    scores = []
    for i, smiles in enumerate(smiles_list):
        mol = Chem.MolFromSmiles(smiles)
        score = QED.qed(mol)
...
def calculatePass_ro5(smiles_list):
    scores = []
    for i, smiles in enumerate(smiles_list):
        mol = Chem.MolFromSmiles(smiles)
        flag = Lipinski.NumHDonors(mol) <= 5 and Lipinski.NumHAcceptors(mol) <= 10 and Descriptors.MolWt(mol) <= 500 and Crippen.MolLogP(mol) <= 5
        if flag:
            score = 1
        else:
            score = 0
...
def calculateHBD(smiles_list):
    scores = []
    for i, smiles in enumerate(smiles_list):
        mol = Chem.MolFromSmiles(smiles)
        score = Lipinski.NumHDonors(mol)
        scores.append(score)
    return np.float32(scores)

def calculateHBA(smiles_list):
    scores = []
    for i, smiles in enumerate(smiles_list):
        mol = Chem.MolFromSmiles(smiles)
        score = Lipinski.NumHAcceptors(mol)
        scores.append(score)
    return np.float32(scores)

def calculateTPSA(smiles_list):
    scores = []
    for i, smiles in enumerate(smiles_list):
        mol = Chem.MolFromSmiles(smiles)
        score = rdMolDescriptors.CalcTPSA(mol)
        scores.append(score)
    return np.float32(scores)

def calculatenRot(smiles_list):
    scores = []
    for i, smiles in enumerate(smiles_list):
        mol = Chem.MolFromSmiles(smiles)
        score = Lipinski.NumRotatableBonds(mol)
        scores.append(score)
    return np.float32(scores)

def calculateUnique(smiles_list):
    unique_list = list(set(smiles_list))
    return len(unique_list) / len(smiles_list)
    
    
def calculateNovelty(smiles_list): # check from terminal output: unique/mean
    return 1
