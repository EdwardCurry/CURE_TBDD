...
from metrics.PRnet_sys import PRnet_metrics, prnet_2_csv, PRnet_metrics_top_k

from rdkit import Chem, DataStructs
from rdkit.Chem import MACCSkeys

from search import get_top_k_sim_smiles, get_top_k_sim_smiles_list, get_goal_radio
...
class Graph_DiT(pl.LightningModule):
    ...
    def on_validation_epoch_start(self) -> None:
        ...
        if "smiles" not in self.guidance_target:
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if self.val_counter % self.cfg.general.sample_every_val == 0 and self.val_counter > 1:
            ...
            elif "smiles" in self.guidance_target:
                valid, validity, nc_validity, num_components, valid_smiles, covered_atoms = costom_compute_relaxed_validity(self.dataset_info.atom_decoder, samples, self.dataset_info.ensure_connected) 
    ...
    def smiles_to_atom_and_edge_types(self, smiles):
        mol = Chem.MolFromSmiles(smiles)
        if mol is None:
            raise ValueError(f"smiles are not valid: {smiles}")
        ...
        for atom in mol.GetAtoms():
            ...
            if symbol == 'As' or symbol == 'Te':
                raise ValueError(f"Unsupport type: {symbol}")
            if symbol not in self.ATOM_TYPES:
                symbol = 'H'
            atom_types.append(self.ATOM_TYPES[symbol])
        
        edge_types = np.zeros((n_atoms, n_atoms), dtype=np.int64)
        ...
        for bond in mol.GetBonds():
            ...
            if bond_type in self.BOND_TYPES:
                edge_types[i, j] = self.BOND_TYPES[bond_type]
            else:
                edge_types[i, j] = 0 
        
        atom_types = torch.tensor(atom_types, dtype=torch.long).to("cuda")
        edge_types = torch.tensor(edge_types, dtype=torch.long).to("cuda")
    ...
    def test_step(self, data, i):
        ...
        self.test_all_data.append(data)
        return {'loss': nll}



    def on_test_epoch_end_org(self) -> None:
        self.cfg.general.final_model_samples_to_generate = 1000
    ...
    def calculate_max_maccs_similarity(self, reference_csv, query_smiles_list, output_csv="similarity_results.csv"):
        try:
            ref_df = pd.read_csv(reference_csv)
            ref_df.dropna(subset=['smiles'], inplace=True) 
        except Exception as e:
            raise ValueError(f"Loading CSV files failed: {str(e)}")
        
        if 'smiles' not in ref_df.columns:
            raise ValueError("Smiles are not found")
        ...
        for smi in ref_df['smiles']:
            try:
                ...
                if mol:
                    ...
                    print(f"Unvalid SMILES: {smi}")
            except Exception as e:
                print(f"Error SMILES: {smi} - {str(e)}")
        
        if not ref_fps:
            raise ValueError("There are no valid SMILES in the reference set for fingerprint generation")
        ...
        for q_smi in query_smiles_list:
            try:
                ...
                if not q_mol:
                    print(f"WARNING: Invalid query SMILES: {q_smi}")
                    max_sim = np.nan
                else:
                    q_fp = MACCSkeys.GenMACCSKeys(q_mol)
                ...
                results.append({'smiles': q_smi, 'max_maccs_similarity': max_sim})
            except Exception as e:
                results.append({'smiles': q_smi, 'max_maccs_similarity': np.nan})
        
        results_df = pd.DataFrame(results)
        results_df.to_csv(output_csv, index=False)
        print(f"Results saved to {output_csv}")

        avg_max_similarity = results_df['max_maccs_similarity'].mean(skipna=True)
        return avg_max_similarity, results_df
    

    def on_test_epoch_end_fix(self, num_nodes_input=None, run_type=None, index_j=None) -> None:
        """ Measure likelihood on a test set and compute stability metrics. """
        metrics = [self.test_nll.compute(), self.test_X_kl.compute(), self.test_E_kl.compute(),
                   self.test_X_logp.compute(), self.test_E_logp.compute()]
        ...
        test_y_collection = torch.cat(self.test_y_collection, dim=0)
        num_examples = test_y_collection.size(0)
        self.cfg.general.final_model_samples_to_generate = samples_left_to_generate = num_examples
        
        for data_bs in self.test_all_data:
            print(f'samples left to generate: {samples_left_to_generate}/'
                f'{self.cfg.general.final_model_samples_to_generate}', end='', flush=True)
            bs = 1 * self.cfg.train.batch_size
            to_generate = min(samples_left_to_generate, bs)
            ...
            num_nodes = []
            if run_type is None:
                for i in range(len(data_bs.ptr)):
                    if i == 0:
                        continue
                    num_nodes.append(data_bs.ptr[i] - data_bs.ptr[i-1])
            else:
                num_nodes = [num_nodes_input] * (len(data_bs.ptr) - 1)
            cur_sample = self.sample_batch(batch_id, to_generate, batch_y, save_final=to_save,
                                            keep_chain=chains_save, number_chain_steps=self.number_chain_steps, num_nodes=num_nodes)
            samples = samples + cur_sample
        ...
        print(f"final Computing sampling metrics...")
        self.sampling_metrics.reset()
        all_smiles = self.sampling_metrics(samples, all_ys, self.name, self.current_epoch, self.val_counter, test=True)
        self.sampling_metrics.reset()
        if "smiles" in self.guidance_target:
            valid, validity, nc_validity, num_components, all_smiles, covered_atoms = costom_compute_relaxed_validity(self.dataset_info.atom_decoder, samples, self.dataset_info.ensure_connected)  # self.dataset_info
        
        org_smiles = []
        org_index = []
        for data_bs in self.test_all_data:
            org_smiles = org_smiles + data_bs.sms
            org_index = org_index + [int(meta) for meta in data_bs.sms_index]
            break
        if "smiles_ge" in self.cfg.dataset.task_name: # Zero-shot Gene inhibitor
            ref_file_path = os.path.join("./DiT/get_feat/ge/org_smiles", self.cfg.dataset.task_name.split("_")[-1].split("+")[0]+"_ref.csv") # replace Gene inhibitor data path(default in ./DiT/get_feat/ge/org_smiles)
            mean_max, results_df = self.calculate_max_maccs_similarity(ref_file_path, all_smiles, 
                    output_csv=os.path.join("./DiT/get_feat/ge/res_maccs", f"similarity_results_{self.cfg.dataset.task_name}_{num_nodes_input}_{index_j}.csv"))
            print(self.cfg.dataset.task_name, mean_max)
            return mean_max, results_df
            
        output_csv_path = f"./smiles_similarity_results_for_{self.cfg.dataset.task_name}_{self.guide_scale}.csv" # replace 
        results_df = compare_smiles_lists(org_smiles, all_smiles, output_csv_path) 

        print(results_df.head())
        print(f"Done.")

        print("calculateQED: ", np.mean(calculateQED(org_smiles)))
        print("calculatePass_ro5: ", np.mean(calculatePass_ro5(org_smiles)))
        print("calculatelogP: ", np.mean(calculatelogP(org_smiles)))
        print("calculateMW: ", np.mean(calculateMW(org_smiles)))
        print("calculateHBA: ", np.mean(calculateHBA(org_smiles)))
        print("calculateTPSA: ", np.mean(calculateTPSA(org_smiles)))
        print("calculatenRot: ", np.mean(calculatenRot(org_smiles)))
        print("calculateHBD: ", np.mean(calculateHBD(org_smiles)))
        ...
        h5ad_file = os.path.join(os.path.dirname(sys.path[0]), f"Perturbation-Response-Prediction-PRnet-0847146/dataset/Lincs_L1000.h5ad") # scTrans-Gen/DiT/Perturbation-Response-Prediction-PRnet-0847146/dataset/Lincs_L1000.h5ad
        ...
        input_csv_file = os.path.join(os.path.dirname(sys.path[0]), f"data/feat/level3_landmark_ctp_cp_amp_33567.csv") # scTrans-Gen/DiT/data/feat/level3_landmark_ctp_cp_amp_33567.csv
        res, dose_dict = check_drug_dose_consistency_with_mode(h5ad_file, input_csv_file)

        if "other" in self.cfg.dataset.task_name:
            feat_true_csv = os.path.join(os.path.dirname(os.path.dirname(sys.path[0])), f"molgene/l1000_smiles_feat_dit_prnet_other.csv")
        elif "cell" in self.cfg.dataset.task_name:
            feat_true_csv = os.path.join(os.path.dirname(os.path.dirname(sys.path[0])), f"molgene/l1000_smiles_feat_dit_prnet_cell.csv")
        elif "drug" in self.cfg.dataset.task_name:
            feat_true_csv = os.path.join(os.path.dirname(os.path.dirname(sys.path[0])), f"molgene/l1000_smiles_feat_dit_prnet_smiles.csv")
        else:
            return
        result_list = PRnet_metrics(org_index, feat_true_csv, dose_dict, all_smiles, PRnet, org_smiles)
        prnet_2_csv(result_list, f"./pr_net_{self.cfg.dataset.task_name}.csv") # comp_org_pre_mse is shown in paper
        ### PRnet ###


        # drug screener
        k = 15
        top_k_result_list, avg_sim = get_top_k_sim_smiles_list(all_smiles, k=k)
        print("avg_sim: ", avg_sim) 
        goal_radio, goal_list = get_goal_radio(top_k_result_list, org_smiles)
        print("goal_radio: ", goal_radio)
        result_list_top_k = PRnet_metrics_top_k(org_index, feat_true_csv, dose_dict, top_k_result_list, PRnet, org_smiles)
        prnet_2_csv(result_list_top_k, f"./test_pr_net_top_k_{self.cfg.dataset.task_name}_{k}.csv")



    def extend_list(self, lst, num):
        if num <= 0:
            return []
        if not lst:
            return [None] * num
        n = len(lst)
        repeated_list = lst * ((num + n - 1) // n)
        return repeated_list[:num]


    def on_test_epoch_end(self) -> None:
        self.on_test_epoch_end_fix()


    def max_mean_ge(self, results_dfs):
        num_rows = len(results_dfs[0])
        max_values = []
        for i in range(num_rows):
            row_similarities = []
            
            for df in results_dfs:
                value = df['max_maccs_similarity'].iloc[i]
                row_similarities.append(value)
            
            if all(pd.isna(val) for val in row_similarities):
                max_values.append(np.nan)
            else:
                max_val = np.nanmax(row_similarities)
                max_values.append(max_val)

        average_max = np.nanmean(max_values) if not all(np.isnan(max_values)) else np.nan
        return average_max
    ...
    def sample_batch(self, batch_id, batch_size, y, keep_chain, number_chain_steps, save_final, num_nodes=None):
        ...
        elif type(num_nodes) == list:
            n_nodes = torch.tensor(num_nodes, device=self.device, dtype=torch.int)
        else:
            assert isinstance(num_nodes, torch.Tensor)
            n_nodes = num_nodes
        n_max = self.max_n_nodes
        if type(num_nodes) == list:
            arange = torch.arange(n_max, device=self.device).unsqueeze(0).expand(n_nodes.shape[0], -1)
        else:
