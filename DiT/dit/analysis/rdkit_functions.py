...
import torch
from metrics.property_metric import calculateSAS, calculateSA, calculateQED, calculatePass_ro5, calculatelogP, calculateMW, calculateHBA, calculateTPSA, calculatenRot, calculateHBD, calculateUnique, calculateNovelty
...
def costom_compute_relaxed_validity(atom_decoder, generated, ensure_connected):  # dataset_info.atom_decoder  samples  dataset_info.ensure_connected
    valid = []
    num_components = []
    all_smiles = []
    valid_mols = []
...
class BasicMolecularMetrics(object):
    ...
    def evaluate(self, generated, targets, ensure_connected, active_atoms=None):  # # samples, all_ys, dataset_info.ensure_connected, dataset_info.active_atoms
        """ generated: list of pairs (positions: n x 3, atom_types: n [int])
            the positions and atom types should already be masked. """
        valid, validity, nc_validity, num_components, all_smiles, covered_atoms = self.compute_relaxed_validity(generated, ensure_connected=ensure_connected)  # samples  dataset_info.ensure_connected
            ...
            if self.task_evaluator is not None:
                ...
                for i, name in enumerate(evaluation_list):
                    if name == 'scs':
                        ...
                        scores = calculateSA(valid)
                    elif name == "QED_score":
                        scores = calculateQED(valid)
                    elif name == "pass_ro5":
                        scores = calculatePass_ro5(valid)
                    elif name == "logP":
                        scores = calculatelogP(valid)
                    elif name == "MW":
                        scores = calculateMW(valid)
                    elif name == "HBD":
                        scores = calculateHBD(valid)
                    elif name == "HBA":
                        scores = calculateHBA(valid)
                    elif name == "TPSA":
                        scores = calculateTPSA(valid)
                    elif name == "nRot":
                        scores = calculatenRot(valid)
                    elif name == "unique":
                        scores = calculateUnique(valid)
                    elif name == "novelty":
                        scores = calculateNovelty(valid)
                    else:
                        # breakpoint()
                    ...
                    elif name in ["sas", "SA_score", "QED_score", "pass_ro5", "logP", "MW", "HBD", "HBA", "TPSA", "nRot","unique","novelty"]:
                        dist_metrics[f'{name}/mean'] = np.mean(scores)
                    else:
                        true_y = targets[:, i]
...
def compute_molecular_metrics(molecule_list, targets, train_smiles, stat_ref, dataset_info, task_evaluator, comput_config):
    ...
    evaluated_res = metrics.evaluate(molecule_list, targets, ensure_connected, active_atoms)  # samples, all_ys, dataset_info.ensure_connected, dataset_info.active_atoms
