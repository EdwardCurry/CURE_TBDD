...
from models.conditions import TimestepEmbedder, CategoricalEmbedder, ClusterContinuousEmbedder, ClusterContinuousEmbedder_mutidim
...
class Denoiser(nn.Module):
    def __init__(
        ...
        if task_type == "others":
            self.y_embedding_list.append(ClusterContinuousEmbedder_mutidim(2048, hidden_size, drop_condition)) # 2048 978 250
        else:
    ...
    def forward(self, x, e, node_mask, y, t, unconditioned):
        ...
        force_drop_id = torch.zeros_like(y.sum(-1)) # b all 0
        force_drop_id[torch.isnan(y.sum(-1))] = 1 # b null 1
        if unconditioned:
            force_drop_id = torch.ones_like(y[:, 0]) # b all 1
        ...
        elif self.task_type == "others":
            c2 = self.y_embedding_list[0](y[:, :], self.training, force_drop_id)
            # for i in range(1, self.ydim):
            #     if i == 1:
            #         c2 = self.y_embedding_list[i-1](y[:, :2], self.training, force_drop_id)
            #     else:
