...
class molgene_feat_1000(nn.Module):
    def __init__(self, config=None, input_len=978, embed_dim=16, num_heads=4, decode_embed_dim=2048, activate='relu'):
        ...
        self.fc = nn.Linear(2 * embed_dim * input_len, 250)
        self.Tanh = nn.Tanh()
...
class molgene_feat_graph(nn.Module):
    def __init__(self, config=None, input_len=978, embed_dim=16, num_heads=4, h=32, w=32, activate='relu'):
        super().__init__()
        if config:
            print("Using config")
            input_len = config["molgene_feat_info"]["input_len"]
        ...
        # self.pos_encoder = Learnable1DPositionEncoding(embed_dim, input_len)
        
        self.final_attn = MultiHeadCrossAttention(embed_dim * 2, num_heads)
        self.fc = nn.Linear(2 * embed_dim * input_len, h*w)
        self.Tanh = nn.Tanh()
    ...
    def forward(self, a_, b_):
        ...
        out = self.fc(out)
        device = out.device
        out = out.reshape(-1, self.h, self.w).to(device)
...
class molgene_feat_cat(nn.Module):
    def __init__(self, config=None, input_len=978, embed_dim=16, num_heads=4, decode_embed_dim=2048, activate='relu'):
        super().__init__()
        if config:
            print("Using config")
        ...
        self.fc = nn.Linear(2 * input_len, decode_embed_dim)
        self.Tanh = nn.Tanh()
        self.activate = activate
        # self.relu = F.relu()
    ...
    def forward(self, a_, b_):
        ...
        out = torch.cat([a_, b_], dim=2)
        
        out = out.flatten(1)  # (B, 2  * input_len)
        out = self.fc(out)
        if self.activate == "relu":
            out = F.relu(out)
        elif self.activate == "Tanh":
