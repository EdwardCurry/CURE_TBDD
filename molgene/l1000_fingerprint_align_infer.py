...
from module.mol_dit_feat import mol_dit_feat_org_hgraph_l1000_model
import torch
...
def drug_graph_collate(batch):
    ...
    return org_img, drug_img, None, None, smiles_string
...
def alignment_loss(A, B, alpha=0.4, beta=0.1, weight_init=1): # 1
    ...
    total_loss = loss_pos + alpha * loss_neg # + sparsity_loss
    return total_loss
...
def append_tensor_and_smiles_to_csv(tensor, smiles_list, output_path):
    ...
    assert tensor.shape[0] == len(smiles_list), \
        f"Tensor batch size ({tensor.shape[0]}) 和 SMILES列表长度 ({len(smiles_list)}) 不匹配"
...
def feat_to_csv(cp_batch, ctl_batch, smiles_batch, filename, mode='a'):

    b = len(smiles_batch)
    if cp_batch.shape[0] != b or ctl_batch.shape[0] != b:
        raise ValueError("Error: "
                         f"smiles_batch={b}, "
                         f"cp_batch={cp_batch.shape[0]}, "
                         f"ctl_batch={ctl_batch.shape[0]}")
    
    cp_batch = cp_batch.to("cpu").numpy()
    ctl_batch = ctl_batch.to("cpu").numpy()
    cp_columns = [f"cp_{i}" for i in range(cp_batch.shape[1])]
    ctl_columns = [f"ctl_{i}" for i in range(ctl_batch.shape[1])]
    all_columns = ["smiles"] + cp_columns + ctl_columns
    
    data_dict = {"smiles": smiles_batch}
    
    for i, col in enumerate(cp_columns):
        data_dict[col] = cp_batch[:, i]
    
    for i, col in enumerate(ctl_columns):
        data_dict[col] = ctl_batch[:, i]
...
if __name__ == "__main__":
    ...
    train_dataset = CpCtlDataset_solo_mask(cp_path, ctl_path, data_type="smiles")
    ...
    model = mol_dit_feat_org_hgraph_l1000_model(config, embed_dim=32).to(device)
    checkpoint = torch.load("") # replace with your path
    ...
    with torch.no_grad():
        for batch in tqdm(train_loader):
            ...
            append_tensor_and_smiles_to_csv(pre_vec, smiles_string, "l1000_smiles_feat_dit_smiles.csv")

            feat_to_csv(x_drug, x_org, smiles_string, "l1000_smiles_feat_dit_prnet_smiles.csv")
